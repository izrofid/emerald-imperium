...
YAML_OUTPUT="${DEX_DIR}/items.yaml"
JSON_OUTPUT="${DEX_DIR}/items.json"
...
echo "Extracting item data..."
echo "Output: $JSON_OUTPUT"

# Run the test and extract data between markers
cd "$PROJECT_ROOT" || exit 1
make -j4 check TESTS="Extract Items" | \
    awk '/===ITEMS DEBUG OUTPUT START===/{flag=1; next} /===ITEMS DEBUG OUTPUT END===/{flag=0} flag' > "$YAML_OUTPUT"
...
echo "Done! item data saved to $JSON_OUTPUT"