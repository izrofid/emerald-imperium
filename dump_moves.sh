...
YAML_OUTPUT="${DEX_DIR}/moves.yaml"
JSON_OUTPUT="${DEX_DIR}/moves.json"
...
make -j4 check TESTS="Extract Moves" | \
    awk '/===MOVES DEBUG OUTPUT START===/{flag=1; next} /===MOVES DEBUG OUTPUT END===/{flag=0} flag' > "$YAML_OUTPUT"
